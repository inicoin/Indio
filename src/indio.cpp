...
// Indio: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateIndioNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetIndioBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, uint256 prevHash)
{
    ...
    if (!consensusParams.fSimplifiedRewards)
    {
        ...
        CAmount maxReward = (600000 >> halvings) - 1;
        int rand = generateMTRandom(seed, maxReward);

        return (1 + rand) * COIN;
    } else if (nHeight < (6 * consensusParams.nSubsidyHalvingInterval)) {
        // New-style constant rewards for each halving interval
        return (300000 * COIN) >> halvings;
    } else {
        // Constant inflation
        return 25000 * COIN;
    }
...
int64_t GetIndioDustFee(const std::vector<CTxOut> &vout, CFeeRate &baseFeeRate) {
    int64_t nFee = 0;
