# Indiocoin Core 1.8.1

Indiocoin Core 1.8.1 is primarily a bugfix release, bringing Indiocoin Core in
line with Bitcoin 0.9.3. Indiocoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Indiocoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Indiocoin Core clients
* Use HTTPS with indiochain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/indio/indio/issues/134).
...
This only affects values significantly below 1 Indio, however merchants may wish to round to
the nearest Indio on all transactions for now.
